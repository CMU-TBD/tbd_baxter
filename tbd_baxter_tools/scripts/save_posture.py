...
import rospy
import argparse
from tbd_baxter_tools.motion import save_posture
...
def main():
    ...
    # parse the arguments
    parser = argparse.ArgumentParser(description="Script that saves the current position of Baxter's arm as a list of joint" +
                                     " position enable callback with move_to_posture.py")
    parser.add_argument("posture_name", help="Name of the posture to be called in the future", type=str)
    parser.add_argument("-b", "--button", help="If included, will only save the posture after pressing the big spin wheel" +
                        " on either arms", dest="button", action="store_true", default=False)
    ...
    rospy.loginfo(f"Saved posture:{args.posture_name}")
